...
> **产品决策（2026-06-18）**：主导航命名为 **「项目」**（不用「AI 写文档」）；生成后 **直接保存** 到项目目录，不要求用户再点「保存」，避免遗漏；用户可在 Obsidian 或后续 App 内编辑已保存文档。
...
- AI 流式生成完成后，**自动写入** `{projects_dir}/{slug}.md`（无需二次确认保存）
- 保存成功后界面提示：「已保存至 xxx.md」并提供 **[在 Evidence 中打开]** **[在 Obsidian 中打开]**（若可检测）
...
| 检查 | 失败处理 |
|------|----------|
| 引用存在于 paragraphs.json | 标红警告；文件仍已保存，用户可事后修改 |
| 中文提炼未标成「原文」 | 警告 |
| B-实务决策 缺操作结论 | 警告 |

**后续修改：** v1 不强制 App 内编辑器；用户主要在 Obsidian / 本地 Markdown 工具中改稿；Evidence 分屏用于对照准则核对引用。
