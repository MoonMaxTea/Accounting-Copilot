...
- 输入：问题 + 可选事实 JSON
- 生成：流式显示生成过程（用户可见进度，但 **无需点保存**）
- 保存：**生成结束即自动写入** `projects_dir/{slug}.md`
- 反馈：toast「已保存至 …」+ 引用校验报告（有问题标红/标黄，**不阻止已写入的文件**）
- 后续：用户可在 Obsidian 改稿；Evidence 分屏打开同一文件对照准则
...
```
用户输入
  → 加载 writing-spec/项目编写说明.md + SKILL.md
  → 构建 system prompt（约束：仅引用 paragraphs.json 中 current 条目）
  → 调用 OpenAI API（流式）
  → 解析输出中的引用标记
  → 自动写入 projects_dir/{slug}.md
  → 展示校验报告（引用是否存在于 pack）
  → 可选：一键跳转 Evidence 打开该文件
```

**保存策略（产品决策）：** 直接保存，无「预览后确认保存」步骤，避免用户忘记保存；校验问题以警告展示，不撤销已写文件。
